...
def ndcg(relevance, k):
    dcg = sum(
        (1 / math.log2(i + 2)) for i, r in enumerate(relevance[:k]) if r == 1
    )

    ideal = sorted(relevance, reverse=True)
    idcg = sum(
        (1 / math.log2(i + 2)) for i, r in enumerate(ideal[:k]) if r == 1
    )
...
def semantic_similarity(model, preds, refs):
    emb_pred = model.encode(preds, normalize_embeddings=True, convert_to_tensor=True)
    emb_ref = model.encode(refs, normalize_embeddings=True, convert_to_tensor=True)
